  ...
  const fetchMoments = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/request/retrieve/moments?latitude=${latitude}&longitude=${longitude}`,
        {
          method: "GET",
          headers: {
            "X-App-Secret": "smartboyakriti",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Server ${response.status}: ${text || response.status}`,
        );
      }
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <Text style={{ color: "#757575" }}>
                        {info.commentCount > 1 ? "Comments" : "Comment"}
                      </Text>
                    </View>
                  </TouchableOpacity>
                  ...
                  >
                    <Animated.View
                      style={{
                        transform: [
                          {
                            scale: activePost === info.postUuid ? scaleAnim : 1,
                          },
                        ],
                      }}
                    >
                      <Ionicons
                        name={info.interested ? "heart" : "heart-outline"}
                        size={24}
                        color={info.interested ? "#E30103" : "#757575"}
                      />
                    </Animated.View>
                    ...
                    >
                      {info.interestedCount > 0 && (
                        <Text style={{ color: "#757575" }}>
                          {info.interestedCount}
                        </Text>
                      )}
                      <Text style={{ color: "#757575" }}>
                        {info.interestedCount > 1 ? "Likes" : "Like"}
                      </Text>
                    </View>
                  </TouchableOpacity>
                </View>
